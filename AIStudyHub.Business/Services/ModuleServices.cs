...
public sealed class FlashcardService : CrudService<FlashcardResponseDto, CreateFlashcardRequestDto, UpdateFlashcardRequestDto>, IFlashcardService
{
    ...
    public async Task<IReadOnlyList<FlashcardResponseDto>> CreateBulkAsync(IReadOnlyList<CreateFlashcardRequestDto> requests, CancellationToken cancellationToken = default)
    {
        var results = new List<FlashcardResponseDto>();
        ...
        foreach (var request in requests)
        {
            ...
            var flashcard = new Data.Entities.Flashcard
            {
                Id = Guid.NewGuid(),
            ...
            await _unitOfWork.Flashcards.AddAsync(flashcard, cancellationToken);
            await _unitOfWork.SaveChangesAsync(cancellationToken);
            results.Add(_mapper.Map<FlashcardResponseDto>(flashcard));
        }
