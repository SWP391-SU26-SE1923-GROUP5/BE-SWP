...
public sealed class UnverifiedAccountCleanupService : BackgroundService
{
    ...
    private async Task CleanupUnverifiedAccountsAsync(CancellationToken cancellationToken)
    {
        ...
        // Delete FlashcardDecks first (Flashcards will be cascade deleted)
        var flashcardDecks = await dbContext.FlashcardDecks
            .Where(d => documentIds.Contains(d.DocumentId))
            .ToListAsync(cancellationToken);
        dbContext.FlashcardDecks.RemoveRange(flashcardDecks);
